...
public class ZTD {
    ...
    void tap(int x, int y) {
                ...
                for (Cannon c : cannons)
                    if (c.hitbox().contains(x, y)) {
                        if (c.disabled) { UI.money += 5; c.active = false; }
                        return;
                    }

                if(UI.money >= (Tables.values.get("unlock_" + current_type) == null ? 15 : Tables.values.get("unlock_" + current_type)) && buildable(x, y)) {
                    UI.money -= (Tables.values.get("unlock_" + current_type) == null ? 15 : Tables.values.get("unlock_" + current_type));
    ...
    public void spawn_zombies(){
        ...
        for (int i = 0; i < 5 * UI.wave; i ++)
            if(UI.wave == 1) zombies.add(new Zombie("zombie", 1024 + i * 50, r.nextInt(400)));
            else if(UI.wave == 2) zombies.add(new Zombie("zombie", 1024 + i * 50, r.nextInt(400)));
            else if(UI.wave == 3) zombies.add(new Zombie("zombie", 1024 + i * 50, r.nextInt(400)));
            else zombies.add(new Zombie("zombie", 1024 + i * 50, r.nextInt(400)));
    }
}
